...
const runtime=`export * from '/packages/portal-mojo/src/client/runtime.ts'; export const useCan=()=>({can:globalThis.__pushTests.can,me:{id:1}}); export const mojoList=async()=>({rows:[]}); export const mojoCall=async(path,opts={})=>{const f=globalThis.__pushTests;f.calls.push({path,...opts});return f.reply;};`;
...
try {
 ...
 await open();fixture.reply=new Promise(resolve=>{finishSend=resolve;});await click('Send notification');fixture.can=false;await render(null);await React.act(async()=>finishSend({status:true,data:accepted}));assert(!document.body.textContent.includes('Accepted by FCM'),'Late send result is hidden after permission loss');assert(!button('Cancel').disabled,'A settled operation still unlocks dismissal after permission loss');await click('Cancel');
