...
async function messagingFetch(path:string,opts:MockFetchOpts):Promise<unknown|undefined>{
    ...
    if(actionMatch){
        ...
        const action=actionMatch[2];row.modified=Math.floor(Date.now()/1000);
        if(action==='audit'){row.status=row.name.includes('manual')?'missing':'ready';return {status:true,data:{domain:row.name,region:row.region,status:row.status,audit_pass:row.status==='ready',checks:{ses_production_access:true,ses_verified:row.status==='ready',dkim_verified:row.status==='ready',notification_topics_ok:true,unknown_provider_check:'drop-me'},items:[{resource:'SES identity',status:row.status,desired:{secret:'drop'}},{resource:'DKIM',status:row.status}],recommendations:row.status==='ready'?[]:['Publish the returned verification records.'],provider:{raw:'drop'}}};}
        ...
        row.status='ready';return {status:true,data:{domain:row.name,region:row.region,topic_arns:{bounce:'arn:aws:sns:mock:bounce'},receipt_rule:'mojo-inbound',rule_set:'mojo',notes:['SES/SNS resources reconciled. No DNS writes were made.']}};
    }
    ...
    if(path==='/api/aws/email/send'){
        if(!hasGlobalPermission(caller,EMAIL_GRANTS))return permissionDenied();if(method!=='POST')return {status:false,error:'Method not allowed',error_code:405};const body=opts.body??{};const mailbox=db.emailMailboxes.find(v=>v.email===String(body.from_email));if(!mailbox)return {status:false,error:'Mailbox not found',error_code:404};if(!mailbox.allow_outbound)return permissionDenied();const now=Math.floor(Date.now()/1000);const recipients=Array.isArray(body.to)?body.to.map(String):[];const failed=recipients.some(v=>v.includes('fail'));const row:MockSentMessage={id:Math.max(0,...db.sentMessages.map(v=>v.id))+1,created:now,modified:now,mailbox:mailbox.id,to_addresses:recipients,cc_addresses:Array.isArray(body.cc)?body.cc.map(String):[],bcc_addresses:Array.isArray(body.bcc)?body.bcc.map(String):[],subject:String(body.subject??body.template_name??''),status:failed?'failed':'sending',ses_message_id:failed?null:`mock-${now}`,status_reason:failed?'Provider rejected the recipient':null,body_text:body.body_text?String(body.body_text):null,body_html:body.body_html?String(body.body_html):null};db.sentMessages.unshift(row);return {status:true,data:sentWire(row,'default')};
    }
