...
function buildIncidentEvents(): MockIncidentEvent[] {
    ...
    rows.push(
        ...
        {
            id: 8985, created: nowSec - 600, level: 8, scope: 'mojosec',
            category: 'mojosec.auth.sudo_command', source_ip: '88.184.56.101',
            hostname: null, uid: null, country_code: null,
            title: 'MojoSec detected auth.sudo_command',
            details: 'MojoSec sensor prod-web-i-04aab759def924cdd reported 1 occurrence(s) of auth.sudo_command. Host recommendation: review (advisory only).',
            model_name: 'mojosec_sensor', model_id: null,
            geo_ip: '88.184.56.101 (Saint-Julien-du-Sault, FR)',
            metadata: {
                mojosec: {
                    kind: 'auth.sudo_command', count: 1,
                    event_id: 'd4dc2901fd65f59196244a9a8e200bfaff9e5a10b1a9f6f1936a79237f39481c',
                    evidence: {
                        cwd: '/opt/api', actor: 'ec2-user',
                        cgroup: '/user.slice/user-1000.slice/session-95740.scope',
                        boot_id: '194f44c3dcf945c08a0c3e076a784ff9',
                        command: '/usr/local/sbin/mojo-firewall-broker',
                        selinux: 'unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023',
                        monotonic: 2767741178039, attribution: 'audit_session', target_user: 'root',
                        command_path: '/usr/local/sbin/mojo-firewall-broker', producer_exe: '/usr/bin/sudo',
                        producer_pid: 2552251, producer_uid: 1000, proof_status: 'partial',
                        systemd_unit: 'session-95740.scope', audit_session: 95740,
                        producer_comm: 'sudo', audit_loginuid: 1000, command_family: 'unknown',
                        receipt_semantics: ['normalize exact IPv4 DROP rules'],
                    },
                    last_seen: '2026-09-09T22:53:46.490459Z',
                    sensor_id: 'prod-web-i-04aab759def924cdd',
                    first_seen: '2026-09-09T22:53:46.490459Z', recommendation: 'review',
                    effective_level: 8, sensor_severity: 'high', protocol_version: 1,
                    installation_key_id: 36,
                    sensor_policy_revision_sha256: 'a071e28caf7e5b8a4657c2c80b2a020495507609bdd869cbcce0bce37109ee15',
                },
            },
            group_id: null, incident: null,
        },
...
async function pushFetch(path:string,opts:MockFetchOpts):Promise<unknown|undefined>{
    ...
    if(path==='/api/account/devices/push/test'){
        if(!['GET','POST'].includes(method))return {status:false,error:'Method not allowed',error_code:405};
        if(!hasGlobalPermission(caller,['send_notifications','comms']))return permissionDenied();
        const body=opts.body??{};const deviceId=method==='GET'?opts.params?.device_id:body.device_id;
        if(typeof deviceId==='boolean'||!/^\d+$/.test(String(deviceId))||!Number.isSafeInteger(Number(deviceId))||Number(deviceId)<=0)return {status:false,error:'A positive registered-device ID is required',error_code:400};
        ...
        const title=body.title??'Push test';const message=body.message??'This is a test notification.';
        if(typeof title!=='string'||!title.trim()||title.length>200||typeof message!=='string'||!message.trim()||message.length>1000)return {status:false,error:'Title and message are required and must fit their limits',error_code:400};
        if(code)return {status:true,data:{...readiness,success:false,outcome:'blocked',delivery_id:null}};
        const now=Math.floor(Date.now()/1000);const deliveryId=Math.max(0,...db.pushDeliveries.map(row=>row.id))+1;
        const messageId=`projects/${config.fcm_project_id}/messages/mock-${deliveryId}`;
        db.pushDeliveries.push({id:deliveryId,created:now,modified:now,user:device.user,device:device.id,template:null,title,body:message,category:'test',action_url:null,data_payload:{},status:'sent',sent_at:now,delivered_at:null,error_message:null,platform_data:{outcome:'accepted',message_id:messageId,config_id:config.id}});
        return {status:true,data:{...readiness,success:true,outcome:'accepted',message:'FCM accepted the notification. Confirm receipt on the device.',message_id:messageId,delivery_id:deliveryId}};
    }
    const testMatch=path.match(/^\/api\/account\/devices\/push\/config\/(\d+)\/test$/);
    if(testMatch){
        if(method!=='POST')return {status:false,error:'Method not allowed',error_code:405};
        if(!hasGlobalPermission(caller,PUSH_CONFIG_TEST_GRANTS))return permissionDenied();
        const row=db.pushConfigs.find(item=>item.id===Number(testMatch[1]));if(!row)return {status:false,error:'Push configuration not found',error_code:404};
        if(!db.pushConfigSecrets.has(row.id))return {status:false,error:'No FCM service account configured',error_code:400,data:{success:false,outcome:'blocked',error_code:'missing_credentials',validation_only:true}};
        return {status:true,data:{success:true,outcome:'validated',message:'FCM authenticated and validated this project. No notification was delivered.',message_id:`projects/${row.fcm_project_id}/messages/mock-check`,test_mode:row.test_mode,validation_only:true,fcm_version:'v1'}};
    }
