...
async function messagingFetch(path:string,opts:MockFetchOpts):Promise<unknown|undefined>{
    ...
    if(actionMatch){
        ...
        return {status:true,data:{domain:row.name,region:row.region,status:row.status,topic_arns:{bounce:'arn:aws:sns:mock:bounce'},receipt_rule:'mojo-inbound',rule_set:'mojo',notes:['SES/SNS resources reconciled. No DNS writes were made.']}};
    }
