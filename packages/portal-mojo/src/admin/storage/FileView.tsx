...
function RenditionRows({ rows, canManage, pollingDisabled, onRegenerate }: { rows: FileRenditionRow[]; canManage: boolean; pollingDisabled: boolean; onRegenerate: (roles: string[]) => void }) {
    if (!rows.length) return <p className="dim">No renditions have appeared yet.</p>;
    return <div className="storage-renditions">{rows.map((row) => <div className="storage-rendition" key={`${row.role}:${row.id}`}><div><b>{row.role}</b><small>{row.content_type} · {row.width && row.height ? `${row.width}×${row.height} · ` : ''}{row.file_size == null ? 'unknown size' : fmt.filesize(row.file_size)} · {row.upload_status}</small></div><div className="storage-action-row">{isSafeCapabilityUrl(row.url) && <><button className="btn-icon" title="Open rendition" onClick={() => openCapabilityUrl(row.url!)}><i className="bi bi-box-arrow-up-right" /></button><button className="btn-icon" title="Download rendition" onClick={() => openCapabilityUrl(row.url!, true)}><i className="bi bi-download" /></button></>}{canManage && <button className="btn-icon" disabled={pollingDisabled} title={`Regenerate ${row.role}`} onClick={() => onRegenerate([row.role])}><i className="bi bi-arrow-clockwise" /></button>}</div></div>)}</div>;
}
...
export function FileView({ id, onClose }: { id: number; onClose: () => void }) {
    ...
    const startPoll = async (before: string, targetRoles: readonly string[] | null) => {
        ...
        try {
            const reason = await pollRenditionConvergence({ fileId: id, beforeSignature: before, targetRoles, isCurrent: () => alive.current && pollGeneration.current === generation, fetch: async () => { const result = await query.refetch(); if (!result.data) throw new Error('File refresh returned no record'); return result.data; } });
            if (alive.current && pollGeneration.current === generation) setPollState(reason);
        } catch (error) {
            if (alive.current && pollGeneration.current === generation) { setPollState('timeout'); toast.error(error instanceof Error ? error.message : 'Rendition refresh failed'); }
        } finally { if (pollGeneration.current === generation) pollInFlightRef.current = false; }
    };
    ...
    useEffect(() => {
        ...
        void startPoll(renditionTargetSignature(file, null), null);
    }, [file?.id]);
    ...
    const runRegenerate = async (roles: string[] = []) => {
        if (regenerationBusyRef.current || pollInFlightRef.current || pollState === 'polling') return;
        regenerationBusyRef.current = true;
        ...
        try {
            ...
            await regenerate.mutateAsync({ id, payload: normalized.length ? normalized : true });
            await startPoll(before, targetRoles);
