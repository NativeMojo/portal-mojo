...
function RenditionRows({ rows, onRegenerate }: { rows: FileRenditionRow[]; onRegenerate: (roles: string[]) => void }) {
    if (!rows.length) return <p className="dim">No renditions have appeared yet.</p>;
    return <div className="storage-renditions">{rows.map((row) => <div className="storage-rendition" key={`${row.role}:${row.id}`}><div><b>{row.role}</b><small>{row.content_type} · {row.width && row.height ? `${row.width}×${row.height} · ` : ''}{row.file_size == null ? 'unknown size' : fmt.filesize(row.file_size)} · {row.upload_status}</small></div><div className="storage-action-row">{isSafeCapabilityUrl(row.url) && <><button className="btn-icon" title="Open rendition" onClick={() => openCapabilityUrl(row.url!)}><i className="bi bi-box-arrow-up-right" /></button><button className="btn-icon" title="Download rendition" onClick={() => openCapabilityUrl(row.url!, true)}><i className="bi bi-download" /></button></>}<button className="btn-icon" title={`Regenerate ${row.role}`} onClick={() => onRegenerate([row.role])}><i className="bi bi-arrow-clockwise" /></button></div></div>)}</div>;
}
