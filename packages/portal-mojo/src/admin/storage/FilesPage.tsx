...
export function FilesPage() {
    ...
    const batches: BatchAction<FileRow>[] = [
        ...
        { key: 'download', label: 'Download selected', confirm: false, eligible: (row) => Boolean(row.url), run: async (row) => { if (!openCapabilityUrl(row.url ?? '', true)) throw new Error(`${row.filename} has no safe download URL`); } },
