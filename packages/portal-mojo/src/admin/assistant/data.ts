...
export function projectReply(value: unknown): AssistantReply | null { const raw = obj(value); const conversationId = raw && number(raw.conversation_id); if (!raw || conversationId == null || typeof raw.response !== 'string') return null; const toolCount = Array.isArray(raw.tool_calls_made) ? Math.min(raw.tool_calls_made.length, 32) : Math.min(Math.max(0, number(raw.tool_calls_made) ?? 0), 32); return { response: text(raw.response, 20_000), conversation_id: conversationId, tool_calls_made: toolCount, duration_ms: number(raw.duration_ms) ?? 0, blocks: projectBlocks(raw.blocks) }; }
