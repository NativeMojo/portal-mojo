...
import {
    sanitizeDomainRow, sanitizeMailboxRow, sanitizePublicMessageRow,
...
export interface BasicRelation { id: number; name?: string; email?: string }
...
export interface MailboxRow {
    id: number; created: number; modified: number; email: string; domain: BasicRelation | number | null;
...
export interface SentMessageRow {
    id: number; created: number; modified: number; mailbox: BasicRelation | number | null;
...
export interface PublicMessageRow {
    ...
    name: string; email: string; subject: string; group: BasicRelation | number | null;
...
export const EmailDomainModel = defineModel<EmailDomainRow>({ name: 'email-domain', endpoint: '/api/aws/email/domain', permissions: { view: EMAIL_ADMIN_PERMISSIONS, manage: EMAIL_ADMIN_PERMISSIONS, delete: EMAIL_ADMIN_PERMISSIONS }, normalizeListParams: (p) => allow(p, 'default', ['region', 'status', 'receiving_enabled', 'dns_mode'], ['name','region','status','receiving_enabled','dns_mode','created','modified'], 'name'), sanitizeRow: sanitizeDomainRow });
