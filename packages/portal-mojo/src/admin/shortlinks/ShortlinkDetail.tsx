...
export function ShortlinkDetail({ id, onClose }: { id: number; onClose: () => void }) {
    ...
    const canManage = useCan(SHORTLINK_MANAGE_PERMISSIONS).can;
    ...
    const act = async (action: () => Promise<unknown>, success: string): Promise<boolean> => {
        if (!canManage || pending || retryBlocked) return false;
        setPending(true);
    ...
    return <DetailView title={row.code} subtitle={row.source || 'No source label'} icon="bi-link-45deg"
        ...
        active={canManage ? { value: row.is_active, disabled: pending || retryBlocked, onChange: next => void act(() => setShortlinkActive(queryClient, row, next), next ? 'Shortlink reactivated' : 'Shortlink deactivated') } : undefined}
        contextMenu={canManage ? [{ label: row.is_active ? 'Deactivate' : 'Reactivate', disabled: pending || retryBlocked, onSelect: () => void act(() => setShortlinkActive(queryClient, row, !row.is_active), 'Shortlink updated') }] : []}
