...
function ConfigDetail({ id, onClose }: { id: number; onClose: () => void }) {
    ...
    return <DetailView
        ...
        contextMenu={canManage ? [{ label: 'Edit configuration', disabled: testing, onSelect: edit }, { label: 'Test stored connection', disabled: testing, onSelect: () => void test() }, { label: row.is_active ? 'Deactivate' : 'Reactivate', disabled: saving, onSelect: () => void toggle(!row.is_active) }] : []}
        sections={[
