// Country centroid table — ported from web-mojo
// src/extensions/map/countryCentroids.js (itself auto-generated from that
// repo's countries.geojson, a FeatureCollection of Points). Coordinate
// values are VERBATIM; only the key set changed, and only because it had
// to.
//
// The source has 249 rows but 244 distinct ISO2 keys: three codes carry
// several territories, and in a JS object literal the LAST one wins. That
// silently made `ES` resolve to Canarias rather than Spain — web-mojo's
// geofence country picker listed "Canarias" and Spanish metrics plotted in
// the Canary Islands. Duplicate keys are also a hard TS error (TS1117), so
// the table could not be transcribed as-is regardless.
//
// Resolution: the PRIMARY territory keeps the key; the 5 secondary rows are
// dropped, recorded here in full so nothing is lost and a future
// subdivision-aware table can restore them:
//
//   BQ  Saba                   lat  17.632512616389718  lng -63.23739481909494
//   BQ  Saint Eustatius        lat  17.4919042294197    lng -62.978230589445026
//   ES  Canarias               lat  28.297665106525546  lng -16.53799441021647
//   TF  Juan De Nova Island    lat -17.06449193630804   lng  42.74374761089645
//   TF  Glorioso Islands       lat -11.566224871643417  lng  47.290948081543384
//
// Kept: BQ → Bonaire, ES → Spain, TF → French Southern Territories.

/** One row of the centroid table: display name + the point to plot. */
...
/** ISO2 → centroid. 244 entries; the framework's only country-name source. */
...
/**
 * ISO2 (any case) → centroid, or `null` when the code is unknown. Unlike the
 * source this never throws on a non-string; callers feed it wire data.
 */
...
/** ISO2 → display name. Unknown codes pass through UPPERCASED, never blank. */
export function countryName(code: string | null | undefined): string {
    if (!code) return '';
    const upper = String(code).toUpperCase();
    return COUNTRY_CENTROIDS[upper]?.name ?? upper;
}

/**
 * `{value, label}` options for country pickers, sorted by `localeCompare` on
 * the label — the exact derivation web-mojo's GeofenceRuleForm.js:13-17 did
 * inline, promoted to a toolkit export so the geofence UI has one source.
 */
export const COUNTRY_OPTIONS: readonly { value: string; label: string }[] = Object.entries(COUNTRY_CENTROIDS)
