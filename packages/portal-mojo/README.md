...
| `portal-mojo/client/runtime` | Runtime client surface without eager mock-test controls. The mock transport itself loads on the first mock request. |
...
| `portal-mojo/ui/shell` | Narrow application-shell primitives: theme, guards, menus/sidebar, right panel, modal/toast hosts, password helpers, and formatters. |
...
| `portal-mojo/admin/registry` | The stable built-in `ADMIN_SECTIONS` roster without broad page barrels. It installs the optional Email→DNS adapter synchronously so communications routes work before DNS navigation. |
| `portal-mojo/admin/assistant/launcher` | The lightweight shell launcher without the Assistant page/data barrel. |
