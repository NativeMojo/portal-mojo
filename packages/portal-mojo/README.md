...
- **Theme tokens + component CSS.** Components render semantic classes
  ...
  This includes the `.guardrail*` rules behind `confirmGuardrail`
  (reference: `apps/portal/src/theme/guardrail.css`) — the package ships no
  CSS, so an app that opens a guardrail must carry that block.
